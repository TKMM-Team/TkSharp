...
public static class TkKeyUtils
{
    ...
    public static bool TryGetKeys(string? sdCardRootPath, [MaybeNullWhen(false)] out KeySet keys)
    {
        ...
        foreach (var possibleKeyLocation in _possibleKeyLocations) {
    ...
    {
        var keysFile = Path.Combine(target, "prod.keys");
        if (!File.Exists(keysFile)) {
            return null;
        }
        
        var titleKeysFile = Path.Combine(target, "title.keys");
        var legacyKeysFile = Path.Combine(target, "title.keys_autogenerated");
        
        KeySet keys = new();

        if (File.Exists(legacyKeysFile) && File.Exists(titleKeysFile)) {
            var combinedFile = Path.Combine(Path.GetTempPath(), "combined_title.keys");
    ...
    {
        var switchFolder = Path.Combine(sdCardRootPath, "switch");
