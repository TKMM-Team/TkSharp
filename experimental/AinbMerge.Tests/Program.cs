...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        ...
        Run("native merge preserves newly added parameter types", () =>
        {
            AinbCodec codec = new(); var b = Graph();
            var low = Update(b, 1, n => n with { Properties = n.Properties.Append(new AinbProperty(AinbDataType.Int, "ExtraInt", new AinbInt(7))).ToAinbList() });
            var high = Update(b, 1, n => n with { Properties = n.Properties.Append(new AinbProperty(AinbDataType.Bool, "ExtraBool", new AinbBool(true))).ToAinbList() });
            var result = new AinbBinaryMerger(codec).Merge(codec.Write(b), [codec.Write(low), codec.Write(high)]);
            Equal(AinbMergeStatus.Merged, result.Report.Status);
            Equal(4, codec.Read(result.Bytes).Nodes[1].Properties.Count);
        });
        Run("native unsupported input uses exact highest-priority bytes", () =>
        {
            AinbCodec codec = new(); byte[] b = codec.Write(Graph()); byte[] unsupported = (byte[])b.Clone();
            int blackboard = BinaryPrimitives.ReadInt32LittleEndian(unsupported.AsSpan(8 * 4));
            BinaryPrimitives.WriteUInt16LittleEndian(unsupported.AsSpan(blackboard), 1);
            var result = new AinbBinaryMerger(codec).Merge(b, [unsupported]);
            Equal(AinbMergeStatus.Fallback, result.Report.Status);
            True(result.Bytes.SequenceEqual(unsupported));
            Throws<AinbMergeNotSupportedException>(() => new AinbBinaryMerger(codec).Merge(b, [unsupported], allowFallback: false));
        });
        Run("unsupported source does not mask another malformed source", () =>
        {
            AinbCodec codec = new(); byte[] b = codec.Write(Graph()); byte[] unsupported = (byte[])b.Clone();
            BinaryPrimitives.WriteUInt32LittleEndian(unsupported.AsSpan(4), 0x404);
            var merger = new AinbBinaryMerger(codec);
            Throws<InvalidDataException>(() => merger.Merge(b, [unsupported, new byte[] { 0 }]));
            Throws<InvalidDataException>(() => merger.Merge(b, [new byte[] { 0 }, unsupported]));
        });
        Run("native no-vanilla fallback keeps highest input unchanged", () =>
        {
            AinbCodec codec = new(); byte[] low = codec.Write(Add(Graph(), "Low")); byte[] high = codec.Write(Add(Graph(), "High"));
            var result = new AinbBinaryMerger(codec).Merge([], [low, high]);
            True(result.Bytes.SequenceEqual(high)); Equal(AinbMergeStatus.Fallback, result.Report.Status);
        });
        Run("native ITkMerger paths and priority agree", () =>
        {
            AinbCodec codec = new(); var b = Graph(); byte[] baseline = codec.Write(b);
            byte[] low = codec.Write(Property(b, 0, 2)); byte[] high = codec.Write(Property(b, 0, 3));
            AinbMerger merger = new(codec); TkChangelogEntry entry = new("AI/Experiment.module.ainb", ChangelogEntryType.Copy, 0, 0);
            using MemoryStream a = new(); using MemoryStream c = new();
            merger.Merge(entry, new ArraySegment<byte>[] { low, high }, baseline, a);
            using var buffers = RentedBuffers<byte>.Allocate(new Stream[] { new MemoryStream(low), new MemoryStream(high) }, disposeStreams: true);
            merger.Merge(entry, buffers, baseline, c);
            True(a.ToArray().SequenceEqual(c.ToArray()));
            Equal(new AinbFloat(3), codec.Read(a.ToArray()).Nodes[1].Properties[0].Value);
            using MemoryStream single = new(); merger.MergeSingle(entry, high, baseline, single);
            True(single.ToArray().SequenceEqual(high));
        });
    ...
    private static void RunFixtures(string root)
    {
        ...
        foreach (var c in cases.RootElement.EnumerateArray()) Run(c.GetProperty("name").GetString()!, () =>
        {
            ...
            {
                "merged" => AinbMergeStatus.Merged,
                "fallback" => AinbMergeStatus.Fallback,
                "input-reused" => AinbMergeStatus.InputReused,
                "unchanged" => AinbMergeStatus.Unchanged,
            ...
            Run("native IO: " + c.GetProperty("name").GetString(), () =>
            {
                AinbCodec native = new();
                var binary = new AinbBinaryMerger(native).Merge(
                    File.ReadAllBytes(Path.Combine(directory, "base.ainb")),
                    [File.ReadAllBytes(Path.Combine(directory, "low.ainb")),
                     File.ReadAllBytes(Path.Combine(directory, "high.ainb"))]);
                Equal(AinbGraphMerger.WithoutGuids(expected), AinbGraphMerger.WithoutGuids(native.Read(binary.Bytes)));
                Equal(status, binary.Report.Status);
                Equal(result.Report.Conflicts.Count, binary.Report.Conflicts.Count);
                Equal(result.Report.Additions.Count, binary.Report.Additions.Count);
                for (int i = 0; i < result.Report.Additions.Count; i++)
                    Equal(result.Report.Additions[i], binary.Report.Additions[i]);
                File.WriteAllBytes(Path.Combine(directory, "csharp-merged.ainb"), binary.Bytes);
            });
        });
    }
    ...
    private static AinbDocument Graph()
    {
        ...
        {
            Type = AinbNodeType.Simultaneous,
    ...
    private static AinbDocument Reorder(AinbDocument d, int[] order)
    {
        ...
        return d with
        {
            Nodes = order.Select(old => d.Nodes[old] with
            {
                Index = map[old],
                Id = Guid.Empty,
                Queries = d.Nodes[old].Queries.Select(Ref).ToAinbList(),
            ...
            Commands = d.Commands.Select(c => c with
            {
                Id = Guid.Empty,
...
// using owned immutable snapshots. Fixture integration tests also use the real codec.
