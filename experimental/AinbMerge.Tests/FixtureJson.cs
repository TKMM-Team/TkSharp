...
internal static class FixtureJson
{
    public static AinbDocument Read(string path)
    {
        ...
        return new() {
            Version = d.GetProperty("Version").GetUInt32(), Name = Text(d, "Filename"), Category = Text(d, "Category"),
            BlackboardId = d.GetProperty("Blackboard ID").GetUInt32(), ParentBlackboardId = d.GetProperty("Parent Blackboard ID").GetUInt32(),
    ...
    private static AinbNode Node(JsonElement n)
    {
        ...
        foreach (var flag in n.GetProperty("Flags").EnumerateArray()) flags |= flag.GetString() switch {
            "Is Query" => AinbNodeFlags.Query, "Is Module" => AinbNodeFlags.Module, "Is Root Node" => AinbNodeFlags.Root,
            _ => throw new InvalidDataException("Unknown node flag in test translator.")
        };
        return new() {
            Type = Enum.Parse<AinbNodeType>(nodeType), Index = n.GetProperty("Node Index").GetInt32(),
            Name = Text(n, "Name"), Id = Guid.Parse(Text(n, "GUID")), Flags = flags,
    ...
    private static AinbParameterFlags Flags(JsonElement p)
    {
        ...
        foreach (var flag in p.GetProperty("Flags").EnumerateArray()) flags |= flag.GetString() switch {
            "Uses Default" => AinbParameterFlags.UsesDefault, "Is Output" => AinbParameterFlags.IsOutput,
    ...
    private static AinbValue Value(AinbDataType type, JsonElement v) => type switch {
        AinbDataType.Int => new AinbInt(v.GetInt32()), AinbDataType.Bool => new AinbBool(v.GetBoolean()),
        AinbDataType.Float => new AinbFloat(v.GetSingle()), AinbDataType.String => new AinbString(v.GetString()!),
